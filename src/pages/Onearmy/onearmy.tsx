...
const ArmyPage: FC = () => {
    ...
    useEffect(() => {
        const fetchArmy = async () => {
            try {
                ...
                const response = await apiService.getArmyById(Number(id))
                setArmy(response)
            } catch (error) {
    ...
    let imgArmy = army.ImageArmyUrl.replace("127.0.0.1", "192.168.56.1")
    return (
        <Layout headerButtons={headerButtons}>
            < div className="wrapper" >
                <h1>{army.NameArmy}</h1>
                ...
                <div className="oneArmySingle">
                    <div className="oasImage">
                        <img
                            src={(imageError || imgArmy == "") ? defaultImage : imgArmy}
                            alt={army.NameArmy}
